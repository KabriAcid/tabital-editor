...
function initDatabase() {
  ...
  const insertWord = db.prepare('INSERT OR IGNORE INTO dictionary (word, definition) VALUES (?, ?)');
  const sampleWords = [
    ['Allah', 'God'],
    ['salaam', 'peace'],
    ['kitaab', 'book'],
      ...
      submenu: [
        ...
        },
        {
          ...
          click: async () => {
            ...
            if (!result.canceled) {
              mainWindow.webContents.send('menu-open-document', result.filePaths[0]);
            }
          }
        },
        {
...
ipcMain.handle('get-dictionary-words', (event, query) => {
  ...
  const stmt = query 
    ? db.prepare('SELECT * FROM dictionary WHERE word LIKE ? ORDER BY word LIMIT 10')
